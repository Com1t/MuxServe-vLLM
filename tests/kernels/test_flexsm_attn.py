...
NUM_BLOCKS = 400  # Arbitrary values for testing
...
def ref_single_query_cached_kv_headwise_attention(
    output: torch.Tensor,
    query: torch.Tensor,
    ...
    for i in range(num_seqs):
        ...
        for j in range(context_len):
            ...
            for head_idx in range(num_kv_heads):
                block_number = int(block_table[head_idx][j // block_size])
                block_offset = j % block_size
                k = key_cache[block_number, :, block_offset, :]
                k = k.reshape(head_size)
                key.append(k)
...
def test_single_query_cached_kv_headwise_attention(
    ...
    for _ in range(num_seqs):
        block_table = [
            random.randint(0, NUM_BLOCKS * num_kv_heads - 1) for _ in range(num_kv_heads * max_num_blocks_per_seq)
        ]
        block_tables.append(block_table)
    block_tables = torch.tensor(block_tables, dtype=torch.int, device="cuda")
    block_tables = block_tables.reshape(num_seqs, num_kv_heads, max_num_blocks_per_seq)
    ...
    key_cache = key_cache.reshape(-1, key_cache.shape[-3], key_cache.shape[-2],
                                  key_cache.shape[-1])
    ...
    ref_single_query_cached_kv_headwise_attention(
        ref_output,
        query,
        num_queries_per_kv,
        key_cache,
